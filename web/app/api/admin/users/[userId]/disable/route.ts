...
function adminError(status: number, code: unknown) {
  if (status === 401) return '请先登录。'
  if (status === 403) return '没有管理员权限。'
  if (status === 404) return '用户不存在。'
  if (code === 'cannot_disable_self') return '不能停用当前登录的管理员账号。'
  if (code === 'cannot_disable_admin') return '不能停用管理员账号。'
  return '停用账号失败，请稍后再试。'
}
...
export async function POST(req: Request, context: RouteContext) {
  ...
  const result = await backendRequest(`/internal/admin/users/${encodeURIComponent(userId)}/disable`, {
    method: 'POST',
  ...
  if (!result.ok) {
    return Response.json(
      { error: adminError(result.status, result.data?.error) },
